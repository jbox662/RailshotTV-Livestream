...
VCamFilter::VCamFilter() : OutputFilter()
{
...
#ifdef _WIN64
	const wchar_t *obs_process = L"RailShotBroadcaster.exe";
#else
	const wchar_t *obs_process = L"RailShotBroadcaster.exe";
#endif
	...
	if (vq) {
		...
		StringCbCat(res_file, sizeof(res_file), L"\\railshot-virtualcam.txt");
	...
	if (new_obs_interval == 0) {
		new_obs_interval = 333333;
	}
