...
STDMETHODIMP VCamFactory::CreateInstance(LPUNKNOWN parent, REFIID, void **p_ptr)
{
	...
	*p_ptr = nullptr;
	...
	if (IsEqualCLSID(cls, CLSID_RailShot_VirtualVideo)) {
...
static bool RegServers(bool reg)
{
	...
	if (reg) {
		return RegServer(CLSID_RailShot_VirtualVideo, L"RailShot Virtual Camera", file);
	} else {
		return UnregServer(CLSID_RailShot_VirtualVideo);
	}
}
...
static bool RegFilters(bool reg)
{
	...
	if (reg) {
		...
		hr = fm->RegisterFilter(CLSID_RailShot_VirtualVideo, L"RailShot Virtual Camera", &moniker,
					&CLSID_VideoInputDeviceCategory, nullptr, &rf2);
		if (FAILED(hr)) {
			return false;
		}
	} else {
		hr = fm->UnregisterFilter(&CLSID_VideoInputDeviceCategory, 0, CLSID_RailShot_VirtualVideo);
		if (FAILED(hr)) {
			return false;
		}
	}
