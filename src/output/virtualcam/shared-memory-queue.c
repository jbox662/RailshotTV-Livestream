...
#define VIDEO_NAME L"RailShotVirtualCamVideo"
...
video_queue_t *video_queue_create(uint32_t cx, uint32_t cy, uint64_t interval)
{
    ...
    DWORD size = sizeof(struct queue_header);
...
video_queue_t *video_queue_open(void)
{
    struct video_queue vq = {0};
    vq.handle = OpenFileMappingW(FILE_MAP_READ | FILE_MAP_WRITE, FALSE, VIDEO_NAME);
    if (!vq.handle) {
        vq.handle = OpenFileMappingW(FILE_MAP_READ, FALSE, VIDEO_NAME);
    }
    if (!vq.handle) {
