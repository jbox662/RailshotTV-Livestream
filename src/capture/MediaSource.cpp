...
bool MediaSource::openMedia() {
    ...
    if (hasVideoStream_) {
        swsCtx_ = sws_getContext(videoCtx_->width, videoCtx_->height, videoCtx_->pix_fmt,
                                 videoCtx_->width, videoCtx_->height, AV_PIX_FMT_NV12,
                                 SWS_BILINEAR, nullptr, nullptr, nullptr);
    }

    Logger::info("MediaSource: opened " + config_.pathOrDeviceId);
    return true;
}
        ...
        if (packet->stream_index == videoStreamIndex_ && videoCtx_) {
            if (avcodec_send_packet(videoCtx_, packet) >= 0) {
                while (avcodec_receive_frame(videoCtx_, frame) >= 0) {
                    ...
                    uint8_t* dstData[4] = {vf.data.data(), vf.data.data() + vf.width * vf.height, nullptr, nullptr};
